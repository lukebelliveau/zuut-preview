...
import { handleDeleteOnKeyDown } from '../../../utils/interactionHandlers';
...
import mixpanelTrack from '../../../utils/mixpanelTrack';
...
const useShoppingCartUrl = () => {
  ...
  if (!growId) return PATH_APP.playground;
  return `/cart?${paramKeys.growId}=${growId}&${
    paramKeys.selectedItems
  }=${encodeURIComponent(JSON.stringify(selectedItemNames))}`;
};
...
export default function InventoryDrawer() {
  ...
  const allItemsSelected =
    selectedIds.length === itemIds.length && selectedIds.length > 0;
      ...
      <Stack>
        ...
        >
          <IconButton onClick={selectButtonOnClick} disabled={items.length < 1}>
            <SelectButtonIcon
              sx={{
                color:
                  allItemsSelected || items.length < 1
                    ? 'default'
        ...
        >
          <Button
            ...
            onClick={() => {
              mixpanelTrack('ZUUT Cart opened', {
