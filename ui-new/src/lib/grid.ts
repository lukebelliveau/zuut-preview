...
export default class Grid {
  ...
  constructor(width: number, length: number, snapInterval: number = feetToMm(0.25)) {
    this.width = width;
    this.length = length;
   ...
   */
  snapOffsetPosition(position: Point, offset: Point, isRotated: boolean): Point {
    if (isRotated) {
      return {
        x: Math.round((position.x - offset.y) / this.snapInterval) * this.snapInterval + offset.y,
        y: Math.round((position.y - offset.x) / this.snapInterval) * this.snapInterval + offset.x,
      };
    }
    return {
      x: Math.round((position.x - offset.x) / this.snapInterval) * this.snapInterval + offset.x,
      y: Math.round((position.y - offset.y) / this.snapInterval) * this.snapInterval + offset.y,
    };
  }
  ...
  get verticalLines(): number[][] {
    ...
    for (let i = 0; i < this.width / this.snapInterval; i++) {
      lines.push([i * this.snapInterval, 0, i * this.snapInterval, this.length]);
    }
    return lines;
  }
