...
import CarbonFilterItem, { CARBON_FILTER_ITEM_TYPE } from './item/carbonFilterItem';
import WaterItem from './item/waterItem';
import ExhaustFanItem, { EXHAUST_FAN_ITEM_TYPE } from './item/exhaustFanItem';
import OscillatingFanItem, { OSCILLATING_FAN_ITEM_TYPE } from './item/oscillatingFanItem';
...
import DehumidifierItem, { DEHUMIDIFIER_ITEM_TYPE } from './item/dehumidifierItem';
import airtableApi from '../airtable/airtableApi';
import { useQuery } from '@tanstack/react-query';
import { potRecordComparator } from '../airtable/pots';
import { PlaceableItemRecord, ItemRecord, isPlaceableItemRecord } from '../airtable/Record';
...
const constructorArgs = (item: ItemRecord): ItemConstructorArgs => {
  const amazonProducts: AmazonProduct[] = item.amazonProducts.map((product) => ({
    recordId: product,
  }));
  ...
  if (isPlaceableItemRecord(item)) {
    ...
    constructorArgs.length = inchesToFeet(feetToMm_REQUIRE_3_INCHES(item.length));
